...
// the whole thing... maybe a dot in the middle." Supports half baths ("one and
// a half" — his own explanation on the call).
...
function bedsBathsText(property) {
  ...
  return `${cap} ${bedsLabel} · ${baths} ${bathsLabel}`
}

// Status is the FIRST thing on the card (spec §4): Available <date> /
// Coming Soon / Lease Signed (heavily bold text, no diagonal stamp — Jake's
// note: "just heavily bold the lease signed").
function statusText(property) {
  if (property.status === 'available') {
    return property.availableDate ? `Available ${property.availableDate}` : 'Available Now'
  }
  // Previously split "Lease Signed" (Cedar) from "Rented" (Kendall) per
  // Shavit's own 7/27 distinction; Jake 7/31 standardized both to one label.
  if (property.status === 'lease-signed') return 'Lease Signed'
  ...
  return (
    <article className="ccard listing-card" id={buffer ? undefined : `home-${property.id}`} {...bufferProps}>
      ...
      <div className="ccard__body">
        {/* 1. Status at the very top — Lease Signed heavily bold, no stamp */}
...
function CaseStudiesSection() {
  // Indiana is live: 1919 Kendall is written up in full at
  // /case-studies/1919-kendall/. Michigan and Ohio are still shells until
  // Shavit supplies numbers and photography (the Ohio Drive folders are empty).
  const slots = [
    { state: 'Michigan', title: 'Case studies coming soon.', blurb: 'Three Hillsdale rebuilds, documented deal by deal.' },
    { state: 'Ohio', title: 'Case studies coming soon.', blurb: 'Cleveland projects, Buckingham first.' },
    { state: 'Indiana', title: '1919 Kendall Street.', blurb: 'A South Bend rebuild taken from $68,400 to a $215,000 appraisal, documented start to finish.', href: '/case-studies/1919-kendall/', cta: 'Read the case study' },
  ]
  return (
    <section className="band" id="case-studies">
      <div className="band__inner">
        ...
        <Carousel label="Deal case studies">
          {slots.map((slot) => (
            <article className="qcard case-card" key={slot.state}>
    ...
    <section className="band" id="contact">
      <div className="band__inner">
        ...
        <div className="section-head section-head--left">
          ...
          <Reveal mode="fade" delay={300}><p className="section-head__sub">One number. One inbox.</p></Reveal>
          <Reveal mode="fade" delay={500}><GoldRule wide /></Reveal>
        </div>
        <div className="reach-grid">
          <article className="qcard">
            <Eyebrow gold>Call or text</Eyebrow>
            <h3 className="qcard__pull">{CONTACT.phone}</h3>
            <p className="qcard__quote">Tenants, sellers, and partners — Hillsdale, Cleveland, and South Bend.</p>
            <a className="btn-base btn-gold" href={CONTACT.phoneHref}>Call Now</a>
          </article>
          <article className="qcard">
            <Eyebrow gold>Email</Eyebrow>
            <h3 className="qcard__pull">SEND A NOTE.</h3>
            <p className="qcard__quote">{CONTACT.email}</p>
            <a className="btn-base btn-ghost" href={CONTACT.emailHref}>Send an Email</a>
          </article>
        </div>
      </div>
    </section>
