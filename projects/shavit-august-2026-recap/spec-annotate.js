...
(function () {
  ...
  buildChrome();
  boot();
  // Public hook: pages that add annotatable elements after load (the Notes tab's DOC view)
  // call window.__annotate.rescan() to mount controls on them and re-render their notes.
  window.__annotate = {
    rescan: function () {
      collect();
      targets.forEach(function (el) { if (!el.hasAttribute("data-annot-key")) mountControls(el); });
      renderAll();
    },
    notesFor: function (key) { return notesFor(key).slice(); },
