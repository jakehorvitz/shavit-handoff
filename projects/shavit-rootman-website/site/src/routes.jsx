...
import { BRAND, CONTACT, SECTION_ROUTES, SITE_URL, pages } from './data.js'
...
function rentalListingSchema() {
  const listings = PROPERTIES
    ...
    .map((property, index) => {
      const residence = {
        // type null = Shavit hasn't confirmed house vs apartment (stjoe-19) —
        // claim only the generic schema.org parent, never a guessed subtype.
        '@type': property.type === 'Single Family' ? 'SingleFamilyResidence'
...
function Seo({ path }) {
  ...
  // A page may override: the section routes (/case-studies) render the same
  // homepage markup, so they canonicalise to '/' rather than publishing a
  // duplicate of it.
  const canonicalTarget = page.canonical || cleanPath
  const url = `${SITE_URL}${canonicalTarget === '/' ? '/' : canonicalTarget}`
...
function Layout() {
  ...
  // Scroll to a section either from a #hash (external/legacy links still land
  // correctly) or from a clean section path like /case-studies.
  useEffect(() => {
    const id = location.hash
      ? decodeURIComponent(location.hash.slice(1))
      : SECTION_ROUTES[clean]
    if (!id) return
...
export const routes = [
  {
    ...
    children: [
      ...
      // Clean path for the case-studies section (Shavit 8/31: drop the "#").
      // Renders the homepage and scrolls to #case-studies on mount; the hash
      // URL keeps working for anything already linking to it.
      { path: 'case-studies', element: <HomePage /> },
