...
// Ported verbatim from Motion.jsx (window-globals -> ES modules).
...
export function Reveal({ as = 'div', mode = 'rise', delay = 0, duration, threshold = 0.3, className = '', style = {}, children, ...rest }) {
...
/* LineReveal — a heading split into lines, each line gets a gold hairline
   reveal under it and rises into place. */
export function LineReveal({
  ...
  const [tick, setTick] = useState(0)  // re-render to add settled class
...
/* SurfaceSweep — a 1px gold hairline that draws left->right when the parent
   section enters view, then fades. */
...
/* ScrollLinkedColor — wraps a string; --reveal-progress goes 0->1 as the
   element scrolls from below the fold to fully in view. */
export function ScrollLinkedColor({ as = 'span', className = '', children, ...rest }) {
  const ref = useRef(null)
  useEffect(() => {
    if (!ref.current) return
    let frame = null
