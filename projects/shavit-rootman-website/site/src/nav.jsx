...
import { BRAND, CONTACT, SECTION_ROUTES, SUBLINE } from './data.js'
...
const PATH_BY_KEY = {
  ...
  indiana: '/#indiana',
  invest: '/#invest',
  // Clean path, not '/#case-studies' — Shavit 8/31 asked for the # to go.
...
export function Nav() {
  ...
  // Jump to top only for standalone pages. Section destinations (a #hash, or a
  // clean section path like /case-studies) do their own scrolling in Layout —
  // scrolling to top here would fight it and land the user on the hero.
  const onNav = (key) => {
    const path = PATH_BY_KEY[key] || '/'
    navigate(path)
    if (!path.includes('#') && !SECTION_ROUTES[path]) window.scrollTo({ top: 0 })
  }
