...
const KB_SLIDES = [
  { key: 'loPresto',     path: 'assets/photos/01_hillsdale_17_lo_presto.jpeg' },
  { key: 'secondChance', path: 'assets/photos/03_second_chance_house.jpeg' },
];
  ...
  return (
    <section className={`hero hero--enter ${blueprint ? 'hero--with-bp' : ''} ${height === '80vh' ? 'hero--80vh' : ''}`}>
      <div className="hero__media" aria-hidden="true">
        <div className={`kb ${paused ? 'kb--paused' : ''}`}>
          {KB_SLIDES.map((sl, i) => (
            <img key={sl.key} src={sl.path} alt="" className="kb__img" loading="eager"
                 style={{ animationDelay: `${i * 7}s, ${i * 7}s` }} />
          ))}
        </div>
      </div>
...
function CtaBand({ eyebrow, h, hLines, actions, videoLabel = 'Cinematic Pan · Dusk Street', src, filter }) {
  return (
    <section className="cta-band hero--enter">
      <div className="cta-band__media">{src ? <Placeholder label={videoLabel} kind="video" src={src} filter={filter} /> : <div style={{position:'absolute',inset:0,background:'radial-gradient(80% 120% at 50% 110%, #1b1b1b 0%, #0a0a0a 70%)'}} />}</div>
