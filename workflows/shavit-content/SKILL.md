...
## Listing deliverables (ported from real-estate-listing, retired 7/29)

Formats on request — every property fact still goes through the registry + verify flow:

- **MLS / listing description** — strongest hook first, paint the lifestyle, then specifics, then a call to act. Tight, vivid, scannable.
- **Property one-pager** — headline + hero image + 4–6 feature bullets + specs block + agent contact. Layout via `ui-ux-pro-max`.
- **Market-update post** — neighborhood trends, a stat, Shavit's read on it. Pull real data via `deep-research` and cite it.
- **Premium microsite** — for a flagship listing, chain `scroll-hero-site` for a cinematic scroll page.

**Fair housing (non-negotiable):** describe the *property and location*, never the ideal
buyer/tenant by race, religion, national origin, sex, familial status, disability, etc.
Blacklisted phrasing: "perfect for families", "great for young professionals", "walking
distance", "safe neighborhood", "exclusive community", or anything implying who is or is
not welcome.
